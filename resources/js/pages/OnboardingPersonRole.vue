...
const props = withDefaults(
    defineProps<{
        personRole?: 'coach' | 'freelancer' | 'consultant' | 'creator' | 'media' | 'public_figure' | null;
    }>(),
    {
        personRole: null,
    },
...
const form = useForm<{
    person_role: 'coach' | 'freelancer' | 'consultant' | 'creator' | 'media' | 'public_figure' | '';
}>({
    person_role: props.personRole || '',
});

const submit = () => {
    if (!form.person_role) return;
    form.post('/onboarding/person-role');
};

const options = [
    { value: 'coach', label: 'Coach / Trainer / Educator', short: 'CO' },
    { value: 'freelancer', label: 'Freelancer / Service Provider', short: 'FR' },
    { value: 'consultant', label: 'Consultant / Expert', short: 'EX' },
    { value: 'creator', label: 'Creator / Influencer', short: 'CR' },
    { value: 'media', label: 'Media Personality', short: 'ME' },
    { value: 'public_figure', label: 'Public Figure / Celebrity', short: 'PF' },
] as const;
</script>
            ...
            <div class="flex flex-1 flex-col items-center justify-center text-center lg:px-4">
                <div class="mb-8 flex w-full justify-center">
                    <div class="relative h-44 w-44">
                        ...
                        >
                            <path d="M44 156c22 20 54 30 96 30 38 0 66-9 84-26" stroke="#E5E7EB" stroke-width="10" stroke-linecap="round" />
                            <path d="M64 140c5-28 23-48 54-60 30-12 61-9 92 8" stroke="#111827" stroke-width="10" stroke-linecap="round" />
                            <path d="M140 62l20-26 8 34" stroke="#A855F7" stroke-width="12" stroke-linecap="round" stroke-linejoin="round" />
                            <path d="M86 150c12 4 24 6 38 6 14 0 27-2 38-6" stroke="#111827" stroke-width="10" stroke-linecap="round" />
                            <circle cx="92" cy="108" r="8" fill="#111827" />
                            <circle cx="144" cy="108" r="8" fill="#111827" />
                            <path d="M110 120c10 6 20 6 30 0" stroke="#111827" stroke-width="8" stroke-linecap="round" />
                        </svg>
                    </div>
                </div>
        ...
        <main class="flex flex-1 items-center justify-center bg-white px-6 py-12 lg:px-12 lg:py-16">
            <div class="w-full max-w-xl">
                <h2 class="mb-4 text-base font-semibold text-neutral-900">
                    What best describes you or the person you're setting this account up for?
                </h2>
                    ...
                    >
                        <div class="flex items-center gap-3">
                            <div class="flex size-9 items-center justify-center rounded-lg bg-neutral-100 text-neutral-700">
                                <span class="text-xs font-bold">{{ option.short }}</span>
                            </div>
                            <span class="text-sm font-medium text-neutral-900">{{ option.label }}</span>
                        </div>
                        <input
                            v-model="form.person_role"
                            type="radio"
                            name="person_role"
                            :value="option.value"
                ...
                <div class="mt-8 flex items-center justify-between">
                    ...
                    <button
                        ...
                        :disabled="form.processing || !form.person_role"
                        @click="submit"
                    >
